const getPrefictedAge = async (name: string) => {
  const res = await fetch(`https://api.agify.io/?name=${name}`)
  return res.json()
}

const getPrefictedGender = async (name: string) => {
  const res = await fetch(`https://api.genderize.io/?name=${name}`)
  return res.json()
}

const getPrefictedCountry = async (name: string) => {
...
export default async function Page({params}: Params) {
  const ageData = getPrefictedAge(params.name)
  const genderData = getPrefictedGender(params.name)
  const countryData = getPrefictedCountry(params.name)
